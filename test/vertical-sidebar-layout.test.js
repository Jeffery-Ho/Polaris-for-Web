...
test("Maker pane 将搜索和列表作为垂直布局左侧整体", () => {
  ...
  const verticalMakerPaneRule = styles.match(/#gpt-paragraph-nav\.is-layout-vertical \.gpt-paragraph-nav__maker-pane \{[^}]*\}/)?.[0] || "";
  assert.match(verticalMakerPaneRule, /justify-self: end;[\s\S]*?width: min\(var\(--gpt-nav-controls-width, 360px\), 100%\);/);
  assert.doesNotMatch(verticalMakerPaneRule, /width: 100%;/);
  assert.doesNotMatch(styles, /#gpt-paragraph-nav\.is-layout-vertical \.gpt-paragraph-nav__marker-row \{[\s\S]*?width: 100%;/);
  assert.match(styles, /\.gpt-paragraph-nav__marker-row \{[\s\S]*?width: min\(var\(--gpt-nav-controls-width, 360px\), calc\(100vw - 48px\)\);/);
