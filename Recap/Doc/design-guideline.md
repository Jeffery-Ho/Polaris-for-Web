...
- 导航和设置入口使用单一双 Tab 玻璃胶囊，默认选中“导航”。
- 再次点击已选中的“导航”Tab 时切换 marker 队列的收起和展开；设置 Tab 通过点击显示配置面板，不依赖悬停。
...
- 设置菜单打开后，按 Escape 或点击胶囊和设置面板外的页面区域应返回“导航”Tab。
...
- 在小红书主站从 `/ai_chat` 切至普通路由时，检查插件根节点立即移除；返回 `/ai_chat` 后只重建一套新导航。
