...
- GitHub Pages 页面路径为 `/support.html`，使用现有 Polaris 图标与浅色玻璃布局。
...
- `support-config.js` 是视频与 PayPal 地址的唯一配置位置：`videoSource` 已指向 `assets/polaris-introduction.mp4`。页面在视频开始前显示加载提示，在缓冲时显示缓冲提示，并在 `canplay` 后显式发起 `autoplay muted loop playsinline` 播放；赞赏按钮已配置为在新标签页直达 `https://paypal.me/jefferyhoHK`。
