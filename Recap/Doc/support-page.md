...
- `support-config.js` 是视频与 PayPal 地址的唯一配置位置：`videoSource` 已指向 `assets/polaris-introduction.mp4`，页面以 `autoplay muted loop playsinline` 播放；赞赏按钮已配置为在新标签页直达 `https://paypal.me/jefferyhoHK`。
